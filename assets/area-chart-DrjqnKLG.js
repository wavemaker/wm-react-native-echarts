import{w as ve,u as ke,a as Ce,b as we,i as qe,V as We,S as Se,c as Te}from"./install-D_wEXfzl.js";import{u as Ae}from"./use-axis-tooltip-I7UAJfpW.js";import{c as Le}from"./axis-tooltip-presets-CGVD2u9r.js";import{r as p,R as fe}from"./iframe-DQBbESXd.js";import{v as Re,c as Xe,g as Ve}from"./axis-B8VaOuT_.js";import{e as je,m as ze,i as Fe}from"./echarts-legend-layout-B14L7phv.js";import{i as Ye}from"./common-WjgByzXx.js";import{i as Ge}from"./install-Bq1Dc6dC.js";import{i as Oe}from"./install-B-FBt8Sx.js";ke([Ye,Ge,Fe,Te,Oe]);const ge=({data:f,width:C=220,height:w=350,type:V="default",step:ye,boundaryGap:E=!1,stack:x,stackNormalize:g=!1,symbol:q,symbolSize:N,areaOpacity:B=.6,areaFill:j="gradient",showXAxis:W=!0,showXAxisTicks:$=!0,showYAxis:S=!0,showYAxisTicks:H=!0,showXAxisSplitLines:J=!0,showYAxisSplitLines:U=!0,grid:_,showLegend:z=!0,legendPosition:F="bottom",showHighlighter:K=!0,tooltip:T="card",xAxisTickLabelFormatter:Y,yAxisTickLabelFormatter:G,xAxisLabel:A,yAxisLabel:L,minX:Q,maxX:Z,intervalX:ee,minY:te,maxY:re,intervalY:ie,onSelect:ne,renderTooltip:R,...ae})=>{const{theme:e}=we(ae.theme,ae.colors),O=p.useRef(null),oe=p.useRef(ne);oe.current=ne;const P=p.useRef({displaySeries:[],categoryAxisData:[]}),le=p.useRef(e.series);le.current=e.series;const se=R!=null||T!=="none",be=p.useMemo(()=>R??(T==="none"?()=>null:Le(T)),[R,T]),{attachAxisTooltipListeners:ue,renderAxisTooltipOverlay:xe}=Ae({active:se,renderTooltip:be,contextRef:P,themeSeriesRef:le,width:C,height:w}),de=V==="smooth",I=V==="step"?ye??"start":!1,s=p.useMemo(()=>{let r=[];if(!Array.isArray(f)||f.length===0)return r;const d=f[0];return typeof d=="number"?r=[{name:"Series 1",data:f.map((t,c)=>[String(c),t])}]:Array.isArray(d)&&d.length===2?r=[{name:"Series 1",data:f}]:r=JSON.parse(JSON.stringify(f)).filter(t=>t.data&&t.data.length>0).map((t,c)=>(typeof t.data[0]=="number"&&(t.data=t.data.map((m,i)=>[String(i),m])),{name:t.name||"Series "+(c+1),data:[...t.data]})),r},[f]),M=p.useMemo(()=>s.some(r=>"name"in r&&r.name),[s]),y=p.useMemo(()=>{var d,t;if(!g||s.length<=1)return s;const r=((t=(d=s[0])==null?void 0:d.data)==null?void 0:t.length)??0;return r===0?s:s.map(c=>{const h=c.data,m=new Array(r).fill(0);s.forEach(o=>{("data"in o?o.data:[]).forEach((n,a)=>{a<r&&(m[a]+=typeof n=="number"?n:n[1])})});const i=h.map((o,l)=>{const n=typeof o=="number"?o:o[1],a=m[l]||1;return a>0?n/a*100:0});return{name:c.name,data:h.map((o,l)=>[typeof o=="number"?String(l):o[0],i[l]])}})},[s,g]),me=p.useMemo(()=>Re({minY:te,maxY:re,intervalY:ie}),[te,re,ie]),u=p.useMemo(()=>Xe({minX:Q,maxX:Z,intervalX:ee}),[Q,Z,ee]),D=p.useMemo(()=>{const r=s.map(d=>d.data.map(t=>t[0])).flat();return Ve(r).map(String)},[s]);P.current={displaySeries:y,categoryAxisData:D};const pe=p.useMemo(()=>{const r={type:"category",data:D,...A!=null&&A!==""&&{name:A,nameLocation:"middle",nameGap:25,nameTextStyle:{color:e.axis.x.tickLabelColor}},axisLabel:{show:W||Y!=null,color:e.axis.x.tickLabelColor,formatter:Y??void 0,...(u==null?void 0:u.axisLabelInterval)!==void 0?{interval:u.axisLabelInterval}:{}},axisLine:W?{show:!0,lineStyle:{color:e.axis.x.lineColor,width:e.axis.x.lineWidth}}:{show:!1},axisTick:{show:W&&$,lineStyle:{color:e.axis.x.tickColor,width:e.axis.x.tickWidth}},splitLine:{show:J,lineStyle:{color:e.axis.x.splitLineColor,width:e.axis.x.splitLineWidth}},boundaryGap:E,...(u==null?void 0:u.min)!==void 0&&{min:u.min},...(u==null?void 0:u.max)!==void 0&&{max:u.max}},d={type:"value",...g&&y.length>1?{min:0,max:100}:me??{},...L!=null&&L!==""&&{name:L,nameLocation:"middle",nameGap:40,nameTextStyle:{color:e.axis.y.tickLabelColor}},axisLabel:{show:S||G!=null,color:e.axis.y.tickLabelColor,formatter:G??(g&&y.length>1?i=>`${i}%`:void 0)},axisLine:S?{show:!0,lineStyle:{color:e.axis.y.lineColor,width:e.axis.y.lineWidth}}:{show:!1},axisTick:{show:S&&H,lineStyle:{color:e.axis.y.tickColor,width:e.axis.y.tickWidth}},splitLine:{show:U,lineStyle:{color:e.axis.y.splitLineColor,width:e.axis.y.splitLineWidth}}},t=z&&M?{data:s.filter(i=>"name"in i&&i.name).map(i=>i.name),...je(F),textStyle:{color:e.legend.textColor,fontSize:e.legend.fontSize},backgroundColor:e.legend.backgroundColor}:void 0,c=y.map((i,o)=>{var he;const l=e.series[o%e.series.length].color,n=((he=e.series[o])==null?void 0:he.lineWidth)??e.series[0].lineWidth??2,a=Math.round(B*255).toString(16).padStart(2,"0"),X=l+a,v=l+"00",k=l+"ff",ce=j==="gradient"?{color:{type:"linear",x:0,y:0,x2:0,y2:1,colorStops:[{offset:0,color:X},{offset:1,color:v}]}}:j==="transparent"?{color:X}:{color:k},b={data:"data"in i?i.data:[],type:"line",symbol:!q||q==="none"?void 0:q,symbolSize:N||8,areaStyle:ce,itemStyle:{color:l},lineStyle:{color:l,width:n},emphasis:K?{focus:"self",scale:!0,symbol:"circle",symbolSize:8,itemStyle:{color:l,borderColor:"#FFFFFF",borderWidth:2}}:{focus:"none",scale:!1,symbolSize:0}};return"name"in i&&i.name&&(b.name=i.name),I!==!1?b.step=I:b.smooth=de,x!==void 0&&x!==!1?b.stack=typeof x=="string"?x:"total":g&&y.length>1&&(b.stack="total"),b}),h={xAxis:r,yAxis:d,series:c,axisPointer:{show:!0,type:"line",snap:!0,lineStyle:{type:"solid",width:1,color:"#00000000",show:!1},label:{show:!1}}};t&&(h.legend=t);const m=ze(_,F,z,M);return m!==void 0&&Object.keys(m).length>0&&(h.grid=m),h},[e,s,D,y,V,de,I,E,x,g,q,N,B,j,W,S,$,H,J,U,_,z,F,M,K,se,Y,G,A,L,me,u]);return p.useEffect(()=>{let r,d=()=>{};if(O.current)try{r=qe(O.current,"light",{width:C,height:w}),r.setOption(pe),d=ue(r);const t=c=>{const h=oe.current;if(typeof h!="function"||c.componentType!=="series")return;const m=c.seriesIndex,i=c.dataIndex;if(typeof m!="number"||typeof i!="number"||i<0)return;const{displaySeries:o,categoryAxisData:l}=P.current,n=o[m];if(!(n!=null&&n.data)||!Array.isArray(n.data))return;const a=n.data[i];if(a===void 0)return;const X=n.name!=null&&n.name!==""?String(n.name):`Series ${m+1}`;let v,k;if(typeof a=="number")v=l[i]??i,k=a;else if(Array.isArray(a)&&a.length>=2)v=a[0],k=Number(a[1]);else return;h({seriesIndex:m,dataIndex:i,seriesName:X,x:v,y:k})};r.on("click",t)}catch(t){console.warn("Chart initialization error:",t)}return()=>{if(d(),r)try{r.dispose()}catch(t){console.warn("Chart disposal error:",t)}}},[pe,C,w,ue]),fe.createElement(We,{style:{width:C,height:w,position:"relative"}},fe.createElement(Se,{ref:O,useRNGH:!0}),xe())},Pe=ve(Ce(ge)),Ue=Object.assign(Pe,{displayName:"AreaChart"});ge.__docgenInfo={description:"",methods:[],displayName:"ChartComponent",props:{width:{required:!1,tsType:{name:"number"},description:`Width of the chart in pixels.
...
@default 'gradient'`,defaultValue:{value:"'gradient'",computed:!1}}}};export{Ue as A};
