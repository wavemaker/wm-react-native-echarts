...
| Item | Pie, Radar, Radial, Scatter, Geo, Heatmap | `usePieItemTooltip`, `useHeatmapItemTooltip`, etc. |
...
Heatmap charts use `useHeatmapItemTooltip` with `HeatmapItemTooltipParams` (`xLabel`, `yLabel`, `value`, indices). ECharts `seriesType` must be `heatmap` in `heatmapItemParamsFromShowTip`. Presets live in `heatmap/tooltip/heatmap-item-tooltip-presets.tsx`.
